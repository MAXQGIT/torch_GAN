...
for epoch in range(opt.n_epochs):
    for i, ((imgs_A, labels_A), (imgs_B, labels_B)) in enumerate(zip(dataloader_A, dataloader_B)):
        ...
        valid =torch.ones(batch_size, *patch,requires_grad=False).to(device)
        fake = torch.zeros(batch_size, *patch,requires_grad=False).to(device)


        # Configure input
        imgs_A = torch.FloatTensor(imgs_A).expand(batch_size, 3, opt.img_size, opt.img_size).to(device)
        labels_A = torch.LongTensor(labels_A).to(device)
        imgs_B = torch.FloatTensor(imgs_B).to(device)
        # -----------------
        #  Train Generator
        # -----------------
        ...
        # Sample noise
        z = torch.FloatTensor(np.random.uniform(-1, 1, (batch_size, opt.latent_dim))).to(device)
