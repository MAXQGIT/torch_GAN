...
def sample_images(batches_done):
    ...
    X1  =torch.FloatTensor(imgs["A"]).to(device)
    X2= torch.FloatTensor(imgs["B"]).to(device)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # Set model input
        X1 = torch.FloatTensor(batch["A"]).to(device)
        X2 = torch.FloatTensor(batch["B"]).to(device)


        # Adversarial ground truths
        valid = torch.ones((X1.size(0), *D1.output_shape),requires_grad=False).to(device)
        fake = torch.zeros((X1.size(0), *D1.output_shape),requires_grad=False).to(device)
