...
def compute_gradient_penalty(D, real_samples, fake_samples):
    ...
    fake = torch.ones(validity.shape,requires_grad=False).to(device)
...
def sample_images(batches_done):
    ...
    real_A = torch.FloatTensor(imgs["A"]).to(device)

    fake_B = G_AB(real_A)
    AB = torch.cat((real_A.data, fake_B.data), -2)
    real_B = torch.FloatTensor(imgs["B"]).to(device)
...
for epoch in range(opt.n_epochs):
    for i, batch in enumerate(dataloader):
        # Configure input
        imgs_A =torch.FloatTensor(batch["A"]).to(device)
        imgs_B =torch.FloatTensor(batch["B"]).to(device)
        # ----------------------
