...
def sample_z(shape=64, latent_dim=10, n_c=10, fix_class=-1, req_grad=False):
    ...
    zn = torch.FloatTensor(0.75 * np.random.normal(0, 1, (shape, latent_dim))).detach().to(device)

    ######### zc, zc_idx variables with grads, and zc to one-hot vector
    # Pure one-hot vector generation
    zc_FT = torch.zeros(shape, n_c).to(device)
    ...
    if (fix_class == -1):
        ...
        zc_idx = torch.tensor(zc_idx).to(device)
        zc_FT = torch.tensor(zc_FT).to(device)

    zc = torch.tensor(zc_FT, requires_grad=req_grad)
...
test_imgs = torch.FloatTensor(test_imgs).to(device)
    ...
    for i, (imgs, itruth_label) in enumerate(dataloader):
        ...
        if (i % n_skip_iter == 0):
            ...
            if wass_metric:
                ...
                # Vanilla GAN loss
                valid = torch.ones(gen_imgs.size(0), 1, requires_grad=False).to(device)
        ...
        if wass_metric:
            ...
            fake_loss = bce_loss(D_gen, valid)
